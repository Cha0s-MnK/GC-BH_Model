...
formed stellar mass inside $R_{\rm UV}=6~{\rm pc}$,
\begin{align}
M_{\rm UV}
&=m_{1500,{\rm comp}}
...
$M_{{\rm UV},{\rm QSO1}}=-15.60$ and define
\begin{align}
S_{\rm UV}
...
reached the nuclear region by the evaluation epoch. The UV aperture is fixed
at
\begin{align}
R_{\rm UV}=6~{\rm pc}.
\end{align}
The corresponding stellar mass is the initially formed stellar mass deposited
...
For the QSO1 consistency test, we adopt
\begin{align}
M_{{\rm UV},{\rm QSO1}}=-15.60
\end{align}
and define the UV residual
\begin{align}
S_{\rm UV}
 =
 \frac{M_{\rm UV}-M_{{\rm UV},{\rm QSO1}}}
      {\sigma_{\rm UV}},
 \qquad
 \sigma_{\rm UV}=0.5~{\rm mag}.
\label{eq:suv}
\end{align}
The UV constraint is used as an independent consistency filter on the compact
GC-inspiral model. It should therefore be interpreted as a comparison of the
predicted stellar component with the observed QSO1 UV magnitude, rather than
as a full fit to the source spectrum.
...
For the compact nuclear component, we combine the abundances of the GC
contributors that have reached the central aperture
$R_{\rm UV}=6~{\rm pc}$. Using the deposited initially formed stellar mass
$w_i$ of each contributor as the weight, we calculate the abundance in linear
oxygen units,
\begin{align}
[\mathrm{O/H}]_{\rm nuc}
 =
 \log_{10}\left[
 \frac{\displaystyle\sum_{i\in{\cal C}_{\rm nuc}}
 w_i\,10^{[\mathrm{O/H}]_i}}
 {\displaystyle\sum_{i\in{\cal C}_{\rm nuc}}w_i}
 \right],
\label{eq:nuclear_oh}
\end{align}
where ${\cal C}_{\rm nuc}$ denotes the set of GC-origin contributors within
the nuclear component. This quantity is interpreted as a mass-weighted
nuclear oxygen-abundance proxy. It connects the metallicity of the gas from
which the GCs formed to the present-day gas measured through the QSO1 narrow
emission lines.
