...
\label{eq:IMBHhigh}
\end{align}
Finally, we retain only seeds with $M_\bullet \geqslant 100~M_\odot$, since we focus on IMBH formation through runaway collisions in dense star clusters. The assigned IMBH mass is stored with the GC and is not changed by later stellar evolution or tidal effect.
...
\begin{align}
R_{\rm e} = \dfrac{j}{2} \sqrt{\dfrac{R_{\rm vir}}{G M_{\rm h}}} ,
\label{eq:R_e}
\end{align}
...
\end{align}
Note that here the Sersic index is always $N_{\rm S, bg} = 2.2$ matched to observations. This is because this is the observed distribution of stellar profile while the previous one is just a guess of the GC formation position. The associated mean background density of the smooth background is
\begin{align}
